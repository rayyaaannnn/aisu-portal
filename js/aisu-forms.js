...
async function submitPrimaryMembership() {
    ...
    const justify = {};
    document.querySelectorAll('[name^="justify-"]').forEach(el => { justify[el.name] = el.value; });
    fd.append('justify_answers', JSON.stringify(justify));

    // File uploads
    const fileFields = { govtid_file: 'pm-govtid-file', payment_proof: 'pm-payment', photo: 'pm-photo', sign: 'pm-sign' };
    ...
    try {
        const res = await fetch('http://localhost:5000/api/members/apply', {
            method: 'POST', body: fd
        });
        ...
        function drawSignAndFinalize(signDataUrl) {
            ...
            if (area) {
                area.innerHTML = `<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:14px 18px;font-size:0.88rem;color:#166534;display:inline-flex;align-items:center;gap:10px;">
                    ...
                    <div>Appointment letter <strong>${fname}</strong> has been downloaded!<br>
...
async function submitStudentMembership() {
    ...
    for (const [k, v] of Object.entries(fields)) fd.append(k, v);
    const payEl = document.getElementById('sm-payment');
    if (payEl && payEl.files[0]) fd.append('payment_proof', payEl.files[0]);

    try {
        const res = await fetch('http://localhost:5000/api/students/apply', { method: 'POST', body: fd });
        const json = await res.json();
